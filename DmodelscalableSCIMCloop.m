...
% - relative measurement to left neighbor
% - relative measurement to right neighbor
%
...
figDir = 'C:\Users\20231656\Desktop\figures_sim\SCI_loop';
...
for i = 1:n
    local{i} = build_local_structure(i, n, Hpos);
end

%% 1. Precomputation of deterministic SCI gain and covariance histories

fprintf('Precomputing deterministic SCI gain schedule...\n');

% SCI keeps two covariance parts:
% P_SCI_d = dependent / potentially correlated part
% P_SCI_i = independent / uncorrelated part
...
for i = 1:n
    % Same convention as the supervisor SCI implementation:
    % initial local uncertainty is stored in the independent part.
    P_SCI_d{i} = zeros(2);
    P_SCI_i{i} = P0_local;
end

% Total covariance bound history
P_hist = zeros(2,2,N+1,n);

% Split covariance histories
...
% K_hist{i,k} has size 2 x m_i:
% column 1 = absolute measurement SCI gain
% columns 2... = sequential relative measurement SCI gains
K_hist = cell(n,N);

% Optimized SCI weights
omega_hist = cell(n,N);
...
for m = 1:M
    ...
    for k = 1:N
        ...
        for i = 1:n
            ...
            % SCI processes measurements sequentially.
            x_i_filt = xpred(idx_i);

            Kmat_i = K_hist{i,k};

            % --------------------------------------------------------
            % 1) Absolute position measurement
            % --------------------------------------------------------
            ...
            % --------------------------------------------------------
            % 2) Relative measurements, processed sequentially
            % --------------------------------------------------------
            ...
            for r = 1:deg_i
                ...
                % Raw edge convention:
                % zrel = p_edge_to - p_edge_from + noise
                %
                % Convert relative measurement into a scalar measurement
                % of p_i using the neighbor prediction.
...
%% 3. Monte Carlo empirical covariance

% For scalability, only diagonal empirical covariance is taken, which is
% enough for 3-sigma bounds.
...
if make_plots
    ...
    % Reset live MATLAB figure style to default and dock figures
    reset(groot);
    ...
    for i = 1:n
        ...
        figure('Name',sprintf('SCI_Satellite_%d_position',i));
    ...
    % Relative position figure for each loop edge
    for e = 1:num_edges
        ...
        figure('Name',sprintf('SCI_Relative_position_satellites_%d_and_%d',i,j));
    ...
    figure('Name','SCI_Velocities_of_all_satellites');
    hold on;
    ...
    % Inputs
    figure('Name','SCI_Control_inputs');
    hold on;
    ...
    for i = 1:n
        ...
        figure('Name',sprintf('SCI_Position_error_satellite_%d',i));
    ...
    for i = 1:n
        ...
        figure('Name',sprintf('SCI_Velocity_error_satellite_%d',i));
    ...
    figure('Name','SCI_Common_mode_position_error');
    ...
    figure('Name','SCI_Relative_error_components');
    hold on;
    ...
    figure('Name','SCI_Absolute_measurement_gain');
    hold on;
    ...
    figure('Name','SCI_Relative_measurement_gains');
    hold on;
    ...
    for i = 1:n
        ...
        figure('Name',sprintf('SCI_Agent_%d_velocity_gain_components',i));
        hold on;
...
if make_plots && save_figures
    ...
    fprintf('Saved %d SCI figures to:\n%s\n', numel(figs), figDir);
...
if save_data
    ...
    dataFile = fullfile(dataDir, sprintf('SCI_loop_sim_data_n_%d_MC_%d.mat', n, M));
    ...
    fprintf('Saved SCI simulation data to:\n%s\n', dataFile);

end

%% Auxiliary functions

function local = build_local_structure(i, n, Hpos)
    ...
    %
    % For agent i:
    % left neighbor is i-1 with wrapping
    % right neighbor is i+1 with wrapping
    ...
    % These signs are mostly stored for clarity and compatibility.
    % Left edge: z = p_i - p_left, coefficient of p_i is +1.
    % Right edge: z = p_right - p_i, coefficient of p_i is -1.
    selfSigns = [+1, -1];
...
function [Kmat, omega_vec, P_d_post, P_i_post] = compute_local_SCI_gain_sequence( ...
    ...
    % Sequential SCI update for one local agent.
    %
    % Measurement order:
    % column 1: own absolute position measurement
    % columns 2...: relative measurements with local neighbors
    ...
    for r = 1:m_i
        ...
        % Current local estimate covariance split z1
        Pz1_d = P_d_cur;
        Pz1_i = P_i_cur;
        ...
        if r == 1
            ...
            % z = p_i + v_abs
            %
            % The measurement noise is independent.
            Pz2_d = 0;
            Pz2_i = sigma_abs^2;

        else

            % Relative measurement converted into own-position estimate:
            % p_i estimate = neighbor prediction +/- relative measurement
            %
            % The neighbor predicted position uncertainty is treated as
            % dependent/possibly correlated. The fresh relative measurement
...
function [K_sci, w_star, P_d_post, P_i_post] = sci_scalar_measurement_gain( ...
    ...
    % Split covariance intersection update for one scalar measurement.
    %
    % z1 is the current local state estimate with covariance split:
    % Pz1_d + Pz1_i
    %
    % z2 is a scalar measurement of H*x with covariance split:
    % Pz2_d + Pz2_i
    %
    % The scalar measurement is either:
    % - own absolute position measurement, or
    % - own-position estimate derived from a relative measurement.
...
function [K_sci, P_d_post, P_i_post] = sci_update_for_weight( ...
    ...
    % SCI inflated split covariances.
    %
    % This follows the structure used in the supervisor code:
    % Pz1(w) = Pz1_d / w + Pz1_i
    % Pz2(w) = Pz2_d / (1-w) + Pz2_i
    ...
    % Split independent covariance update.
    %
    % Matrix version of:
    % Px_i = Px * [inv(P1)*Pz1_i*inv(P1)
    %              + H'*(Pz2_i/P2^2)*H] * Px
...
function P = clean_cov(P)
    ...
    % Remove small negative eigenvalues caused by numerical round-off.
    [V,D] = eig(P);
    d = diag(D);
