...
% Agent-specific gamma values.
% Endpoints and interior agents can be tuned differently.
gamma_vec = 1e-5 * ones(n,1);
gamma_vec(1)   = 1e-4;
gamma_vec(end) = 1e-4;

% For n = 3 this gives:
% gamma1 = 1e-4, gamma2 = 1e-5, gamma3 = 1e-4
...
assert(n >= 2, 'This chain-relative-measurement implementation assumes n >= 2.');
...
for i = 1:n
    local{i} = build_local_structure_chain(i, n, Hpos);
end
...
for k = 1:N
    ...
    if k == 1
        ...
        for i = 1:n
            ...
            [C_all_i, R_i_mv] = build_local_global_measurement_matrix_chain( ...
...
for m = 1:M
    ...
    for k = 1:N
        ...
        for i = 1:n
            ...
            zmeas_i = build_local_zmeas_chain( ...
...
dataDir = 'C:\Users\20231656\Desktop\figures_sim';
if ~exist(dataDir, 'dir')
    mkdir(dataDir);
end

dataFile = fullfile(dataDir, sprintf('OCI_sim_data_n_%d_MC_%d.mat', n, M));
